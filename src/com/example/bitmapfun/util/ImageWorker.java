...
import com.example.imgweb.BuildConfig;
...
public abstract class ImageWorker {
	...
	private ImageCache mImageCache;//关联的图片缓存类
	 ...
	 *            
	 */
	//加载图片的方法，如果缓存中有就从缓存中拿�?
	public void loadImage(Object data, ImageView imageView) {
		Bitmap bitmap = null;

		if (mImageCache != null) {
			bitmap = mImageCache.getBitmapFromMemCache(String.valueOf(data));//从内存中拿出
		}
	...
	public static boolean cancelPotentialWork(Object data, ImageView imageView) {
		final BitmapWorkerTask bitmapWorkerTask = getBitmapWorkerTask(imageView);//获取ImageView对应的Task

		if (bitmapWorkerTask != null) {
			final Object bitmapData = bitmapWorkerTask.data;//获取Task里面的url
			//判断刚获取的获取Task里面的url与实际要加载额url是否�?�?
	 ...
	 */
	//将每个ImageView与BitmapworkTask异步加载任务绑定起来
	...
	public static class AsyncDrawable extends BitmapDrawable {
		private final WeakReference<BitmapWorkerTask> bitmapWorkerTaskReference;//弱引用随时回收内�?
